...
public class MessageController extends AbstractInvController {
    ...
    public void datatable( HttpServletRequest request, HttpServletResponse response, 
        ...
        try {
            ...
            for( Message message : messageList ){
                tmp = new JSONArray();
                for(String column : colArr){
                    if( "messageId".equals(column) ){
                        tmp.put(message.getMessageId());
                    } else if( "messageName".equals(column) ){
                        tmp.put(message.getMessageName());
                    } else if( "messageType".equals(column) ){
                        tmp.put(message.getMessageType());
                    } else if( "version".equals(column) ){
                        tmp.put(message.getVersion());
                    } else if( "messageText".equals(column) ){
                        tmp.put(message.getMessageText());
                    } else if( "irbApproval".equals(column) ){
                        tmp.put(message.getIrbApproval());
                    } else if( "irbApprovalStartDate".equals(column) ){
                        tmp.put(message.getIrbApprovalStartDate());
                    } else if( "irbApprovalEndDate".equals(column) ){
                        tmp.put(message.getIrbApprovalEndDate());
                    } else if( "description".equals(column) ){
                        tmp.put(message.getDescription());
                    } else if( "urls".equals(column)) {
                        urls = "";
                        if("list".equals(display)){
                            urls += "<a href=\"show.html?"+"messageId="+message.getMessageId()+"\">[view]</a>";
                            urls += "<a href=\"edit.html?"+"messageId="+message.getMessageId()+"\">[edit]</a>";
                            urls += "<a href=\"delete.html?"+"messageId="+message.getMessageId()+"\">[delete]</a>";
                        } else {
    ...
    public ModelAndView edit(@RequestParam("messageId") Integer messageId) {
        ModelMap model = new ModelMap();
    ...
    public ModelAndView show(@RequestParam("messageId") Integer messageId) {
        ModelMap model = new ModelMap();
        Message message = invDaoService.getMessageService().findById(messageId);
        model.addAttribute("message",message);
    ...
    public String save(@ModelAttribute("message") Message message) {
        ...
        return "redirect:/inv/message/list.html";
    }
