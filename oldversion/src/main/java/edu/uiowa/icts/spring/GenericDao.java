...
public class GenericDao<Type> implements GenericDaoInterface<Type> {
	...
	public Type findByProperty( String propertyName, Object value ) {
		Type ob = null;
		try {
			Criteria criteria = this.sessionFactory.getCurrentSession().createCriteria( Class.forName( domainName ) );
	...
	public List<Type> list() {
		try {
			return this.sessionFactory.getCurrentSession().createCriteria( Class.forName( getDomainName() ) ).list();
		} catch ( HibernateException e ) {
	...
	public String dump() {
		...
		List<Type> lt = this.sessionFactory.getCurrentSession().createQuery( "from " + getDomainName() ).list();
		for ( Type t : lt ) {
			try {
	...
	@Transactional( readOnly = true )
	private Criteria criteria( GenericDaoListOptions options ) {
		Criteria criteria = null;
		try {

			if ( options.getAlias() != null && !StringUtils.equals( options.getAlias().trim(), "" ) ) {
				criteria = this.sessionFactory.getCurrentSession().createCriteria( Class.forName( domainName ), options.getAlias().trim() );
			} else {
				criteria = this.sessionFactory.getCurrentSession().createCriteria( Class.forName( domainName ) );
			}

			processAliases( criteria, options );

			Dialect dialect = ( (SessionFactoryImplementor) this.sessionFactory ).getDialect();
			ClassMetadata classMetaData = sessionFactory.getClassMetadata( Class.forName( domainName ) );
			...
			addDisjunctions( criteria, options );
	 ...
	 * @param options
	 */
	private void addDisjunctions( Criteria criteria, GenericDaoListOptions options ) {
		if ( options.getDisjunctions() != null && !options.getDisjunctions().isEmpty() ) {
			for ( Disjunction disjunction : options.getDisjunctions() ) {
	 ...
	 * @param options
	 */
	protected void processAliases( Criteria criteria, GenericDaoListOptions options ) {
	 ...
	 * @param classMetaData 
	 */
	protected void addSearchCriteria( Criteria criteria, GenericDaoListOptions options, ClassMetadata classMetaData, Dialect dialect ) {
		if ( options.getSearch() != null && !"".equals( options.getSearch() ) && options.getSearchColumns() != null && !options.getSearchColumns().isEmpty() ) {
			for ( String theSearch : StringUtils.split( options.getSearch().trim(), ' ' ) ) {
				...
				for ( String propertyName : options.getSearchColumns() ) {
					...
					try {
						org.hibernate.type.Type pt = classMetaData.getPropertyType( propertyName );
						if( pt != null ){
							propertyType = pt.getName();
						} else {
							fail = true;
							log.error( "property type not found for [ " + propertyName + " ]" );
						}
					...
					if ( fail ) {
						...
						if ( StringUtils.equalsIgnoreCase( propertyType, "integer" ) || StringUtils.equalsIgnoreCase( propertyType, "boolean" ) || StringUtils.equalsIgnoreCase( propertyType, "float" ) ) {
							...
							// cast it to varchar to avoid errors
							disj.add( new CastAsVarcharLike( propertyName, ( options.isDoubleWildCard() ? "%" : "" ) + options.getIndividualLikes().get( propertyName ) + "%" ) );
						}
					}
				}
				criteria.add( disj );
	 ...
	 * @param classMetaData 
	 */
	protected void addLikeCriteria( Criteria criteria, GenericDaoListOptions options, ClassMetadata classMetaData, Dialect dialect ) {
